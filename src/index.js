...
class Dreamy extends EventEmitter {
	 ...
	 * @example
	 * await Dreamy.set('foo', 'bar'); // true
	 *
	 * await Dreamy.delete('foo'); // true
	 * await Dreamy.delete(['foo', 'fizz']); // [ true, false ]
	 */
	delete(key) {
	 ...
	 * @example
	 * await Dreamy.set('en', 'db');
	 *
	 * const data = await Dreamy.ensure('foo', 'bar');
	 * console.log(data); // 'bar'
	 *
	 * const data = await Dreamy.ensure('en', 'db');
	 * console.log(data); // 'db'
	 */
	async ensure(key, value = null, path = null) {
		if (value === null) {
			throw new TypeError('ensure: Value must be provided.');
		}

		const exists = await this.has(key);
		if (path !== null) {
			if (!exists) throw new Error('ensure: Key does not exist.');
	 ...
	 * @example
	 * await dreamy.set('foo', 'bar');
	 * await dreamy.set('profile', {
	 *   id: 1234567890,
	 *   username: 'user',
	 *   verified: true,
	 *   nil: null,
	 ...
	 * });
	 *
	 * await dreamy.find(v => v === 'bar'); // { key: 'foo', value: 'bar' }
	 * await dreamy.find(v => v.verified === true); // { key: 'profile', value: { ... } }
	 * await dreamy.find(v => v.desc === 'desc'); // undefined
	 */
	 ...
	 * @example
	 * const data = await dreamy.get('foo');
	 * console.log(data); // 'bar'
	 *
	 * // Using path feature
	 * await dreamy.get('profile', 'verified'); // false
	 */
	get(key, path = null) {
		if (typeof key !== 'string') {
			throw new TypeError('get: Key must be a string.');
		}
	 ...
	 * @example
	 * const dreamy = Dreamy.multi(['users', 'members']);
	 * const dreamy = Dreamy.multi(['users', 'members'], {
	 ...
	 * await dreamy.members.set('bar', 'foo');
	 */
	 ...
	 * @example
	 * await dreamy.set('foo', 'bar');
	 * await dreamy.set('total', 400);
	 * await dreamy.set('exists', false);
	 * await dreamy.set('profile', {
	 *   id: 1234567890,
	 *   username: 'user',
	 *   verified: true,
	 *   nil: null
	 * });
	 * await dreamy.set('todo', [ 'Add a authentication system.', 'Refactor the generator' ]);
	 *
	 * await dreamy.set('profile', false, 'verified');
	 * await dreamy.set('profile', 100, 'balance');
	 */
